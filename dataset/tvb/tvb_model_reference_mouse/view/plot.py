...
def multiview(cortex,hemisphere_left,hemisphere_right,data, fig, suptitle='', figsize=(15, 10), CB_position=[0.27, 0.8, 0.5, 0.05], CB_orientation='horizontal', CB_fontsize=10, CB_label='FR (Hz)', fixed_legend=[], **kwds):
    ...
    rm = cs.region_mapping_data.array_data # Arnau:this works, raises error in triangulation
    x, y, z = vtx.T
...
def animation_nuu (cortex,hemisphere_left,hemisphere_right,data,file_name='./movie.mp4',fps=35,my_dpi=100,factor=10, label='FR (Hz)', use_old=False):  
    ...
    for ii in range(np.shape(data)[0]):
        ...
        if not done:
            ...
            # close figure and delete heavy variables, then gc.collect frees memory
            plt.clf() 
            plt.close()
            del fig
            del cax
...
def multiview_one(region_mapping, cortex,hemisphere_left,hemisphere_right,region,data, fig, suptitle='', title='', figsize=(15, 10), **kwds):
    cs = cortex
    vtx = cs.vertices
    tri = cs.triangles
    rm = cs.region_mapping_data.array_data
    x, y, z = vtx.T
...
def multiview_one_bar(cortex,hemisphere_left,hemisphere_right,region, data, fig, suptitle='', title='', CB_position=[], CB_orientation=[],CB_label=[], **kwds):
    cs = cortex
    vtx = cs.vertices
    tri = cs.triangles
    rm = cs.region_mapping_data.array_data
