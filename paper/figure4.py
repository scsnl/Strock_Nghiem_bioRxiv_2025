...
def plot_bar(f, gs, class_attr, highlight = None, legend = False, title = '', test_threshold = None, ylim = None, showlegend = False, max_roi = 14, roi_names = None, path = None, hline = None, titlebrain = '', tick_only_highlight = False, subtitle = None):
    ...
    if not path is None:
        ...
        ax2 = ax.inset_axes([0.6, 0.55, 0.4, 0.35])
        ax2.patch.set_alpha(1.0)
        img = plt.imread(path)
        ax2.imshow(img)
        ax2.text(0.5, 1.05, titlebrain, va = 'bottom', ha = 'center', transform = ax2.transAxes, color = 'C3', usetex = True)
        ax2.axis('off')
    return ax
...
def main(args):
    ...
    highlight_snr = [[((np.isin(np.arange(426), sum([nodes[r] for r in region.split('_')], start = []))), 'C3', 'affected ROIs')] for region in [region1]*3]
    highlight_aff = [[((np.isin(np.arange(426), sum([nodes[r] for r in region.split('_')], start = []))), 'C3', 'affected ROIs')] for region in region2]
    highlight_delta = [[((np.isin(np.arange(426), sum([nodes[r] for r in region.split('_')], start = []))), 'C3', 'affected ROIs')] for region in [region3]*3]
    ...
    roi_names_rsc = rename(roi_names, {"RSPv_left": "RSC-VL", "RSPv_right": "RSC-VR", "RSPagl_left": "RSC-AL", "RSPagl_right": "RSC-AR", "RSPd_left": "RSC-DL", "RSPd_right": "RSC-DR"})
    roi_names_rsc_cg = rename(roi_names, {"RSPv_left": "RSC-VL", "RSPv_right": "RSC-VR", "RSPagl_left": "RSC-AL", "RSPagl_right": "RSC-AR", "RSPd_left": "RSC-DL", "RSPd_right": "RSC-DR", "ACAd_left": "Cg-DL", "ACAv_left": "Cg-VL", "ACAd_right": "Cg-DR", "ACAv_right": "Cg-VR"})
    roi_names_rsc_cg_prl = rename(roi_names, {"RSPv_left": "RSC-VL", "RSPv_right": "RSC-VR", "RSPagl_left": "RSC-AL", "RSPagl_right": "RSC-AR", "RSPd_left": "RSC-DL", "RSPd_right": "RSC-DR", "ACAd_left": "Cg-DL", "ACAv_left": "Cg-VL", "ACAd_right": "Cg-DR", "ACAv_right": "Cg-VR", "PL_left": "PrL-L", "PL_right": "PrL-R"})
    ...
    ylim = 0,110
    ax_A = letter('A',plot_bar)(f, gs[0,0], class_attr_snr[0][1], highlight_snr[0], legend = True, path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = region1.replace('_', ' + '), roi_names = roi_names_rsc, title = f'$\Delta Q_i$ = {qint-qiasd1:.1f}nS, \\textbf{{SNR = {snr1[0]:d}dB}}', subtitle = f'acc = {acc_snr[0]:.0%}, f1 = {performance_snr[0][0]:.0%}, p = {performance_snr[0][1]:.0%}, r = {performance_snr[0][2]:.0%}')
    ax_B = letter('B',plot_bar)(f, gs[0,1], class_attr_snr[1][1], highlight_snr[1], legend = True, path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = region1.replace('_', ' + '), roi_names = roi_names_rsc, title = f'$\Delta Q_i$ = {qint-qiasd1:.1f}nS, \\textbf{{SNR = {snr1[1]:d}dB}}', subtitle = f'acc = {acc_snr[1]:.0%}, f1 = {performance_snr[1][0]:.0%}, p = {performance_snr[1][1]:.0%}, r = {performance_snr[1][2]:.0%}')
    ax_C = letter('C',plot_bar)(f, gs[0,2], class_attr_snr[2][1], highlight_snr[2], legend = True, path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = region1.replace('_', ' + '), roi_names = roi_names_rsc, title = f'$\Delta Q_i$ = {qint-qiasd1:.1f}nS, \\textbf{{SNR = {snr1[2]:d}dB}}',  max_roi = 100, tick_only_highlight = True, subtitle = f'acc = {acc_snr[2]:.0%}, f1 = {performance_snr[2][0]:.0%}, p = {performance_snr[2][1]:.0%}, r = {performance_snr[2][2]:.0%}')

    ax_D = letter('D',plot_bar)(f, gs[1,0], class_attr_aff[0][1], highlight_aff[0], path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = f'\\textbf{{{region2[0].replace("_", " + ")}}}', roi_names = roi_names_rsc, title = f'SNR = {snr2:d}dB, $\Delta Q_i$ = 0.5nS', subtitle = f'acc = {acc_aff[0]:.0%}, f1 = {performance_aff[0][0]:.0%}, p = {performance_aff[0][1]:.0%}, r = {performance_aff[0][2]:.0%}')
    ax_E = letter('E',plot_bar)(f, gs[1,1], class_attr_aff[1][1], highlight_aff[1], path = f'{figure_path}/tvb/tvb_rsc_cg.png', titlebrain = f'\\textbf{{{region2[1].replace("_", " + ")}}}', roi_names = roi_names_rsc_cg, title = f'SNR = {snr2:d}dB, $\Delta Q_i$ = 0.5nS', subtitle = f'acc = {acc_aff[1]:.0%}, f1 = {performance_aff[1][0]:.0%}, p = {performance_aff[1][1]:.0%}, r = {performance_aff[1][2]:.0%}')
    ax_F = letter('F',plot_bar)(f, gs[1,2], class_attr_aff[2][1], highlight_aff[2], path = f'{figure_path}/tvb/tvb_rsc_cg_prl.png', titlebrain = f'\\textbf{{{region2[2].replace("_", " + ")}}}', roi_names = roi_names_rsc_cg_prl, title = f'SNR = {snr2:d}dB, $\Delta Q_i$ = 0.5nS', subtitle = f'acc = {acc_aff[2]:.0%}, f1 = {performance_aff[2][0]:.0%}, p = {performance_aff[2][1]:.0%}, r = {performance_aff[2][2]:.0%}')

    ax_G = letter('G',plot_bar)(f, gs[2,0], class_attr_delta[0][1], highlight_delta[0], path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = region3.replace('_', ' + '), roi_names = roi_names_rsc, title = f'SNR = {snr3:d}dB, $\\boldsymbol{{\Delta Q_i}}$\\textbf{{ = {qint-qiasd3[0]:.1f}nS}}', subtitle = f'acc = {acc_delta[0]:.0%}, f1 = {performance_delta[0][0]:.0%}, p = {performance_delta[0][1]:.0%}, r = {performance_delta[0][2]:.0%}')
    ax_H = letter('H',plot_bar)(f, gs[2,1], class_attr_delta[1][1], highlight_delta[1], path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = region3.replace('_', ' + '), roi_names = roi_names_rsc, title = f'SNR = {snr3:d}dB, $\\boldsymbol{{\Delta Q_i}}$\\textbf{{ = {qint-qiasd3[1]:.1f}nS}}', subtitle = f'acc = {acc_delta[1]:.0%}, f1 = {performance_delta[1][0]:.0%}, p = {performance_delta[1][1]:.0%}, r = {performance_delta[1][2]:.0%}')
    ax_I = letter('I',plot_bar)(f, gs[2,2], class_attr_delta[2][1], highlight_delta[2], path = f'{figure_path}/tvb/tvb_rsc.png', titlebrain = region3.replace('_', ' + '), roi_names = roi_names_rsc, title = f'SNR = {snr3:d}dB, $\\boldsymbol{{\Delta Q_i}}$\\textbf{{ = {qint-qiasd3[2]:.1f}nS}}', subtitle = f'acc = {acc_delta[2]:.0%}, f1 = {performance_delta[2][0]:.0%}, p = {performance_delta[2][1]:.0%}, r = {performance_delta[2][2]:.0%}')
    if method == 'ig':
        f.savefig(f'{figure_path}/figure4.png', dpi = 600)
    else:
        f.savefig(f'{figure_path}/figure4_{method}.png', dpi = 600)
