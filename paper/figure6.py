...
def imshow_class(f, gs, correlation, p, methods, methods_short, stats = [], title = None):
    ...
    for i in range(len(methods)):
        for j in range(len(methods)):
            ax.text(i,j,f'{correlation[i,j]:.2f}', va = 'center', ha = 'center', color =  sm.to_rgba(0) if correlation[i,j]>0.7 else sm.to_rgba(1), weight = 'bold')
    return ax
...
def main(args):
    ...
    figsize, _ws, _hs = get_figsize(ws, wspace, hs, hspace, zoom, bottom = 1.75, top = 4, right = 2.5)
    ...
    class_attrs_data = {m: find_attr(f'{attr_path_data}/ig_baseline/{m}/Whole_model_data_abide_brainnnetome_6_wmcsf.pt') for m in methods}
    correlation_data, p_correlation_data = correlation_attr(class_attrs_data, methods)

    class_attrs_tvb_human = {m: np.stack([find_attr(f'{attr_path_tvb}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise1.0e-04_snr{snr:.2f}/{m}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise1.0e-04_snr{snr:.2f}_epoch*.pt') for snr,region,qiasd in params_tvb_human], axis = 1) for m in methods}
    correlation_tvb_human, p_correlation_tvb_human = correlation_attr(class_attrs_tvb_human, methods)
 
    class_attrs_tvb_mouse = {m: np.stack([find_attr(f'{attr_path_tvb}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise1.0e-04_snr{snr:.2f}/{m}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise1.0e-04_snr{snr:.2f}_epoch*.pt') for snr,region,qiasd in params_tvb_mouse], axis = 1) for m in methods}
    ...
    f.savefig(f'{figure_path}/figure6.png', dpi = 200)
