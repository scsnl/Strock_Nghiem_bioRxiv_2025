...
def main(args):
    ...
    noises = 10.0**np.array([-3, -4, -5])
    params_tvb_human = [ (noise, region, qiasd) for noise in noises for region in regions_human for qiasd in qiasds]
    params_tvb_mouse = [ (noise, region, qiasd) for noise in noises for region in regions_mouse for qiasd in qiasds]
    ...
    methods = ['ig_zerob', 'ig_meanb', 'ig_medb', 'ig_rb_roi']
    method_names = [ 'Zero', 'Mean',  'Median', 'Random Normal']
    method_names_short = ['Zero', 'Mean', 'Median', 'Random Normal']

    class_attrs = {m: [find_attr(f'{attr_path}/ig_baseline/{m}/simple_n100_a{a:d}_delta{d:.2f}_snr{snr:.2f}_epoch*.pt') for (snr,a,d) in params] for m in methods}
    class_attrs = {m: np.stack(class_attr, axis = 0) for m, class_attr in class_attrs.items()}    
    
    f1 = {m: np.array([f1_score(class_attrs[m][i], thresholds, affected[i]) for i in range(len(class_attrs['ig_zerob']))]) for m in methods}
    precision = {m: np.array([precision_score(class_attrs[m][i], thresholds, affected[i]) for i in range(len(class_attrs['ig_zerob']))]) for m in methods}
    recall = {m: np.array([recall_score(class_attrs[m][i], thresholds, affected[i]) for i in range(len(class_attrs['ig_zerob']))]) for m in methods}
    ...
    class_attrs_tvb_human = {m: np.stack([find_attr(f'{attr_path_tvb}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise{noise:.1e}/{m}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise{noise:.1e}_epoch*.pt') for noise,region,qiasd in params_tvb_human], axis = 0) for m in methods}
    correlation_tvb_human, p_correlation_tvb_human = correlation_attr(class_attrs_tvb_human, methods)
    f1_tvb_human = {m: np.array([f1_score(class_attrs_tvb_human[m][i], thresholds, affected_tvb_human[i]) for i in range(len(class_attrs_tvb_human['ig_zerob']))]) for m in methods}
    precision_tvb_human = {m: np.array([precision_score(class_attrs_tvb_human[m][i], thresholds, affected_tvb_human[i]) for i in range(len(class_attrs_tvb_human['ig_zerob']))]) for m in methods}
    recall_tvb_human = {m: np.array([recall_score(class_attrs_tvb_human[m][i], thresholds, affected_tvb_human[i]) for i in range(len(class_attrs_tvb_human['ig_zerob']))]) for m in methods}
    ...
    class_attrs_tvb_mouse = {m: np.stack([find_attr(f'{attr_path_tvb}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise{noise:.1e}/{m}/tvb_{region}_asdQi_{qiasd:.1f}_ntQi_5.0_noise{noise:.1e}_epoch*.pt') for noise,region,qiasd in params_tvb_mouse], axis = 0) for m in methods}
    correlation_tvb_mouse, p_correlation_tvb_mouse = correlation_attr(class_attrs_tvb_mouse, methods)
    f1_tvb_mouse = {m: np.array([f1_score(class_attrs_tvb_mouse[m][i], thresholds, affected_tvb_mouse[i]) for i in range(len(class_attrs_tvb_mouse['ig_zerob']))]) for m in methods}
    precision_tvb_mouse = {m: np.array([precision_score(class_attrs_tvb_mouse[m][i], thresholds, affected_tvb_mouse[i]) for i in range(len(class_attrs_tvb_mouse['ig_zerob']))]) for m in methods}
    recall_tvb_mouse = {m: np.array([recall_score(class_attrs_tvb_mouse[m][i], thresholds, affected_tvb_mouse[i]) for i in range(len(class_attrs_tvb_mouse['ig_zerob']))]) for m in methods}
