...
def main(args):
    ...
    methods = []
    methods += [(f'IG Zero Baseline', f'ig_zerob', IntegratedGradients, {'baselines': (torch.zeros_like(x), torch.zeros_like(cov))})]
    methods += [(f'IG Random Baseline Global', f'ig_rb_global', IntegratedGradients, {'baselines': (torch.normal(mean = float(torch.mean(x)), std = float(torch.std(x)), size = x.shape[1:])[None], torch.normal(mean = float(torch.mean(cov)), std = float(torch.std(cov)), size = cov.shape[1:])[None])})]
    methods += [(f'IG Random Baseline By ROI', f'ig_rb_roi', IntegratedGradients, {'baselines': (torch.distributions.multivariate_normal.MultivariateNormal(torch.mean(x, dim = (0,2)), torch.diag(torch.std(x, dim = (0,2)))).sample(x.shape[2:]).T[None], torch.normal(mean = float(torch.mean(cov)), std = float(torch.std(cov)), size = cov.shape[1:])[None])})]
    methods += [(f'IG Random Baseline By Time & ROI', f'ig_rb_time', IntegratedGradients, {'baselines': (torch.normal(torch.mean(x, dim = 0), torch.std(x, dim = 0))[None], torch.normal(mean = float(torch.mean(cov)), std = float(torch.std(cov)), size = cov.shape[1:])[None])})]
    methods += [(f'IG Median Baseline', f'ig_medb', IntegratedGradients, {'baselines': (torch.median(x, dim = 0, keepdim = True).values, torch.median(cov, dim = 0, keepdim = True).values)})]
    methods += [(f'IG Mean Baseline', f'ig_meanb', IntegratedGradients, {'baselines': (torch.mean(x, dim = 0, keepdim = True), torch.mean(cov, dim = 0, keepdim = True)) })]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--i', metavar = 'I', type = int, nargs = 1, help = 'number of interacting nodes in each class')
    parser.add_argument('--o', metavar = 'O', type = int, nargs = 1, default = [0], help = 'number of overlaping nodes')
