    ...
    for k, attr in class_attr.items():
        ...
        if not highlight is None:
            for i, (idx,_c,_label) in enumerate(highlight):
                ax.bar(feat, attr*idx[k], label = f'Features/ROIs affected', width = 1.0, color = _c, zorder = i+1)
...
def plot_attr_single(path, class_attr, name, label = '', xlabel = 'Features', ylabel = '', c = '0.0', highlight = None):
    n = len(class_attr)
    n_feat = len(class_attr[0])
    f = plt.figure(figsize = (6, 2), layout = 'constrained')
    feat = np.arange(n_feat)
    ax = f.add_subplot(1,1,1)
    attr = class_attr[1].detach().numpy()
    attr = 100*attr/np.max(attr)
    if not highlight is None:
        for i, (idx,_c,_label) in enumerate(highlight):
            ax.bar(feat, attr*idx[1], label = f'Features/ROIs affected', width = 1.0, color = _c, zorder = i+1)
    ax.bar(feat, attr, width = 1.0, color = c, zorder = 0)
    if not highlight is None:
        ax.legend()
    ax.set_xlabel(xlabel)
...
def main(args):
    ...
    for i, epoch in enumerate(args.epochs):
        ...
        for name, tag, method, kwargs in methods:
            ...
            if args.single:
                plot_attr_single(f'{figure_path}/single_attr_{tag}.png', class_attr, name, c = c_not_affected, label = 'ROI not affected', highlight = highlight, xlabel = 'Features/ROIs', ylabel = 'IG')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--single', action='store_true', help='If set, resave even if file exists')
